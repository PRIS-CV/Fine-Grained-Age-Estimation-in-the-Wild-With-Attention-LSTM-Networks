...
local function createModel(opt)
   ...
   local preresnet = torch.load('/home/scw4150/ln/ST/resnet.multiST.torch/ft-101-ror34-lr01-120epoch.t7')
   --print('pre-trained model structure')
  -- print(preresnet)
   local model_new = {}
   model_new = nn.Sequential()
   for i=1,4 do
      model_new:add(preresnet:get(i))
   end

   local spatial1 = networks.new_spatial_transformer('64,300,64', nil,nil,nil, 56, 64, false)
  spatial_params = spatial1:getParameters()
   model_new:add(spatial1)--stn

   --print(spatial_params:size())
   model_new:add(preresnet:get(5))

   --local spatial2 = networks.new_spatial_transformer('64,300,64', nil,nil,nil, 14, 256, false)
   --model_new:add(spatial2)

   for i=6,9 do
	   model_new:add(preresnet:get(i))
   end
print(model_new)
   --model_new:add(nn.View(512))
   local fclast = nn.Linear(512,8)
   fclast.weight:zero()
   --fclast.bias =
   fc_params = fclast:getParameters()
   --print(fc_params:size())
   model_new:add(fclast)
   --print(model_new)
   --for k,v in pairs(model_new:findModules('nn.Linear')) do
      --v.bias:zero()
   --end
   model_new:cuda()
