...
print(model)
   elseif opt.dataset == 'cifar10' then
      ...
      model21:add(layer(basicblock, 16, n))
      ...
      model22:add(layer(basicblock, 32, n,2))
      model1:add(nn.ConcatTable()
             :add(model22)
             :add(shortcut(16,32,2)))
            :add(nn.CAddTable(true))
            :add(ReLU(true))
      model23=nn.Sequential()
      model23:add(layer(basicblock, 64, n,2))
      model1:add(nn.ConcatTable()
             :add(model23)
             :add(shortcut(32,64,2)))
            :add(nn.CAddTable(true))
--            :add(ReLU(true))
      model:add(nn.ConcatTable()
            :add(model1)
            :add(shortcut(16, 64, 4)))
         :add(nn.CAddTable(true))
         :add(ReLU(true))
      model:add(Avg(8, 8, 1, 1))
      model:add(nn.View(64):setNumInputDims(3))
     -- if opt.log=='cifar10test' then
